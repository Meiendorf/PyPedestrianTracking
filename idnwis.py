...
def reinit_tracker(frame, hog, cascade, _type="hog"):
    ...
    if _type == "hog":
        ...
        rects = cascade.detectMultiScale(frame, HAAR_SCALE, HAAR_NEIGHBORS, minSize=(MIN_SIZE,MIN_SIZE), maxSize=(MAX_SIZE,MAX_SIZE))
    #If nonMax needed
...
args.add_argument("--hscale", type=float, default=1.1, help=
    "Коэффициент увеличение скользящего окна для Haar детектора.")
args.add_argument("--hneighbors", type=int, default=6, help=
    "Количество минимальных соседей для KNN классификатора Haar признаков.")
...
HAAR_SCALE = arg['hscale']
HAAR_NEIGHBORS = arg['hneighbors']
...
while True:
    ...
    #Reinitialization, if we didn't find any bboxes, or if detection is needed
